...
class MCMC:
    ...
    def sampler(self, beta):
        ...
        plt.savefig('mcmcresults_final/begin.png')
        plt.clf()
        ...
        plt.savefig('result_Lazer/proposals'+str(beta)+'.png')
        plt.savefig('mcmcresults_final/proposals.svg', format='svg', dpi=600)
        plt.clf()
...
class ParallelTempering:
    ...
    def run_chains (self):
        ...
        while (end < self.NumSamples):
            ...
            g = open("mcmcresults_final/resultspriors_acceptedpercent_80k.txt", "a+")
...
def main():
    ...
    for i in range(6,8): 
        l = "A:\\Work\\Projects\\USyd\\MCMC\\PT\\LDMCMC_timeseries-master\\"
        problem = i
        if problem == 1:
            ...
            name = "Lazer"
        if problem == 2:
            traindata = np.loadtxt(l + "Data_OneStepAhead\\Sunspot\\train.txt")
            testdata = np.loadtxt(l + "Data_OneStepAhead\\Sunspot\\test.txt")  #
            name = "Sunspot"
        if problem == 3:
        ...
        if problem == 7:
            ...
            name = "ACFinance"
        outres = open("result_Sunspot/resultspriors_ld_80k.txt", "a+")
        ...
        plt.savefig('mcmcresults_final/mcmcrestest_'+name+'_ld_80k.png')
        plt.savefig('mcmcresults_final/mcmcrestest_'+str(maxtemp)+'_ldpt_40k.svg', format='svg', dpi=600)
        plt.clf()
        # -----------------------------------------
        ...
        plt.savefig('mcmcresults_final/mcmcrestrain_'+name+'_ld_80k.png')
        plt.savefig('mcmcresults_final/mcmcrestrain_'+str(maxtemp)+'_ldpt_40k.svg', format='svg', dpi=600)
        plt.clf()
        ...
        f = open('mcmcresults_final/ldpt.pckl','wb')
        pickle.dump(pos_w, f, protocol = 2)
        f.close()
        plt.legend(loc='upper right')

        plt.title("Boxplot of Posterior W (weights and biases)")
        plt.savefig('mcmcresults_final/w_pos_'+name+'_ld_80k.png')
        plt.savefig('mcmcresults_final/w_pos_'+str(maxtemp)+'_ldpt_40k.svg', format='svg', dpi=600)
